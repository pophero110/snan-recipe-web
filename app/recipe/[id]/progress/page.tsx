...
export default function CookingProgressPage({ params }: { params: { id: string } }) {
  ...
  const toggleStep = (index: number) => {
    ...
    localStorage.setItem(`cookingProgress-${params.id}`, JSON.stringify(updatedSteps))
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <Link href={`/recipe/${params.id}`} className="text-gray-600 hover:underline mb-4 inline-flex items-center">
        <ArrowLeft size={20} className="mr-2" />
        Back to recipe
      </Link>
      <h1 className="text-3xl font-light mb-6">{recipe.name} - Cooking Progress</h1>
      <div className="mb-6">
        <div className="bg-white p-4 rounded-lg shadow-sm">
          ...
          {recipe.instructions.map((instruction, index) => (
            <div key={index} className="flex items-start mb-4">
              ...
              >
                {completedSteps.includes(index) && <Check size={16} className="text-white" />}
              </button>
              <div className="flex-grow">
                <p className={`${completedSteps.includes(index) ? 'line-through text-gray-400' : 'text-gray-600'}`}>
                  {instruction.text}
                </p>
